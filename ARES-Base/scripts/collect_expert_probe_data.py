...
def main():
    ...
    #instantiate teh MoE layer engine to eval the expert sub networks
    ...
    with torch.no_grad():
        for step, batch in enumerate(dataloader):
            ...
            # Extract base model hidden states across layers
            # Use raw_model forward pass with output_hidden_states=True
            outputs=raw_model(input_ids, output_hidden_states=True)
            all_hidden_states=outputs[1] #tuple of hidden states

            for layer_idx in moe_layers:
                # Hidden state at MoE layer input (before FFN)
                layer_h = all_hidden_states[layer_idx][:, :-1, :] # (B, T-1, H)
